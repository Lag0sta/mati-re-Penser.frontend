...
import {editMarketURLRequest} from '../utils/newActions.js'
import { updatePublicationURL } from "../store/reducers/publication.js";
...
function PublicationMarketEdit({ modalProps, msgProps, setAuthType, marketURL }: props) {
    ...
    for (const pub of publication) {
        console.log("pub12", pub)
            if (!pub.isArchived) {
                latestPublication = pub
            }
    }
    console.log("latestPublication", latestPublication)
    ...
    const handleEditURL = async () => {
        ...
        try {
            ...
            if (!editReviewResponse.result) {
                const errors = JSON.parse(editReviewResponse.error);
                ...
                dispatch(updatePublicationURL({_id: latestPublication._id, lien: url}))
                setAuthType('');
