...
function ArchivedPublicationAdminView({ modalProps, setPublicationID, setAuthType, setBook, isAdminView, setMarketURL }: props) {
    ...
    const handleEditPublicationTxt = () => {
        if (publication) {
            ...
            modalProps.setModalComponent("editArchivedPublicationTxt");
        }
    }
    ...
    const handleEditPublicationImg = () => {
        if (publication) {
            setPublicationID(publication._id)
            modalProps.setIsTextModalOpen(true);
            modalProps.setModalComponent("editArchivedPublicationImg");
        }
    }

    const handleUnArchive = () => {
        modalProps.setModalComponent("aAStatus")
        modalProps.setIsModalOpen(true)
        setAuthType("aAStatus")
    }
    ...
    function handleClickEditURL() {
        modalProps.setModalComponent("archivedEditMarketURL")
        modalProps.setIsModalOpen(true)
                ...
                <div className="flex w-full justify-between ">
                    ...
                    <div className="mr-4">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="font-bold text-xs text-red-700 text-center border border-2 border-red-600 rounded-md p-1 m-1 hover:bg-red-400 hover:text-white"
                                onClick={handleUnArchive}>DESARCHIVER</span>
                        </div>
                        {publication ? (
                            <img className="object-contain  rounded-md border border-3 border-red-600 " src={publication.img} alt="couverture du livre" />
                        ) : (
...
export default ArchivedPublicationAdminView