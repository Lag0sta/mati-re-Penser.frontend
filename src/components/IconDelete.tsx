...
import { getID } from "../store/reducers/reviews_id.js"; 
...
function IconResponse({ modalProps, iconOrigin, keyNumber, setKeyNumber, topic, thread, index }: props) {
    ...
    console.log("iconOrigin", iconOrigin)
    ...
    const handleDelete = (index: number) => {
        ...
        if (iconOrigin === "review") {
            console.log("xclipx23")
            modalProps.setModalComponent("deleteReview")
            modalProps.setIsModalOpen(true);
            console.log("reviewIDtopic", topic)
            const reviewToDelete = topic[index];
            console.log("reviewToDelete", reviewToDelete)
            dispatch(getID({ id: reviewToDelete._id }))

            console.log("reviewToDeleteID", reviewToDelete._id)
        }
    }
